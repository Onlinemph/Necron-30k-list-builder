...
for (const u of out.units) {
  ...
  for (const m of u.models || []) for (const w of m.wargear || []) if (!hasWeapon(w) && !ruleKeys.has(norm(w)) && !core.has(norm(w))) warn.push(`${u.id}: wargear "${w}" has no weapon profile or wargear text`);
...
for (const l of out.lists) for (const it of l.items) if (!weaponKeys.has(wkey(it.name)) && !weaponKeys.has(wkey(it.name).replace(/^twin(-linked)? /, '')) && !ruleKeys.has(norm(it.name))) warn.push(`list ${l.id}: "${it.name}" has no profile or text`);
