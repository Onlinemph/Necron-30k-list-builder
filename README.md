...
- Rules that depend on the rest of the army. The importer converts each unit again under every condition BSData ties it to: a configuration choice, another unit or upgrade in the army, or the detachment it sits in. The differences become "only with X" and "not with X" checks. Blackshields' Oaths of Moment change units' options, Militia Provenances of War swap weapon lists, a Legiones Auxilia designation takes away its Advanced Reaction, Panoply of Old asks which Legion, and incompatible Oaths rule each other out.
- Detachments that only exist with something else in the army, such as the Tank Commander detachments (a Tank Commander) and Solar Auxilia's Helot Tercio (the Oblitum Pattern Cohort).
- Prime Advantages that add a slot: Rewards of Treachery (Alpha Legion) and Exemplars of the Legions (Shattered Legions) add a slot of your chosen role that only other Legions' units fill, and Logisticae adds a Transport or Heavy Transport slot.
- Questoris Familia's own chart: a Knight Households Primary Detachment of four Prime Lord of War slots that only take Household Rank Prime Advantages, and Additional detachments (Armiger Talon, Automata Talon, Support Banner, Yeomanry Mesnie), one per Scion Aspirant, Preceptor, Scion Martial or Lord Scion, or per matching Household Paradigm. The Yeomanry Mesnie offers the Solar Auxilia and Militia units it allows, and only it does.
- Allies: "Add an allied army…" in the Add detachment menu puts in an Allied Detachment (2 Command, 1 Prime, and 4 Troops) from another BSData army and reloads with its units. Its Command slots unlock allied Auxiliary Detachments (the core ones and the ally's own), allied units only go in allied detachments, they use their own army's Prime Advantages, and Knights join through a Knight Lord of War detachment. Share links and saved lists bring their allies with them.
- Mechanicum trait detachments (The Heart of Power, Command Maniple…) check for a High Command model with the right trait.
